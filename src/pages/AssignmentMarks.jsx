...
const AssignmentMarks = () => {
  ...
  const [showMarksSection, setShowMarksSection] = useState(false); // New state for toggling marks section
  ...
  const handleViewMarks = () => {
    if (showMarksSection) {
      // If marks section is already shown, hide it
      setShowMarksSection(false);
    } else {
      // Otherwise, show marks section and fetch marks data
      const fetchedMarks = [
    ...
    <div>
      <Box sx={{ padding: 3, boxShadow: 3 }}>
        ...
        <Box component="form" sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          >
            {showMarksSection ? 'Hide Marks' : 'View Marks'}
          </Button>
        </Box>
        {showMarksSection && marks && (
          <>
            <Box sx={{ marginTop: 3 }}>
